    ...
    async def create_command(self, ctx):
        ...
        class CommandCreationButton(discord.ui.View):
            ...
            async def create_button(self, interaction, button):
                modal = CommandCreationModal(self.cog)
        ...
        class CommandCreationModal(discord.ui.Modal, title="Create Custom Command"):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                if is_global and command_name in self.cog.commands_data.get("global", {}):
                    await interaction.followup.send(f"A global command named `{command_name}` already exists.", ephemeral=True)
                    return
                
                if not is_global and server_id in self.cog.commands_data.get("servers", {}) and command_name in self.cog.commands_data["servers"][server_id]:
                ...
                if is_global:
                    if "global" not in self.cog.commands_data:
                        self.cog.commands_data["global"] = {}
                    self.cog.commands_data["global"][command_name] = command_data
                else:
                    if "servers" not in self.cog.commands_data:
                        self.cog.commands_data["servers"] = {}
                    if server_id not in self.cog.commands_data["servers"]:
                        self.cog.commands_data["servers"][server_id] = {}
                    self.cog.commands_data["servers"][server_id][command_name] = command_data
                
                # Sauvegarder et enregistrer la commande
                self.cog.save_commands()
                self.cog.register_command(command_name, command_data, is_global, server_id if not is_global else None)
...
class EnhancedCommandBrowser:
    def __init__(self, cog, interaction, commands_categories, sort_method="name"):
        ...
        self.current_page = 0
        self.commands_per_page = 5  # Réduits pour plus de lisibilité et d'espace pour les détails
