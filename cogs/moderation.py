...
class Moderation(commands.Cog, name="moderation"):
    ...
    async def archive(self, context: Context, limit: int = 10) -> None:
        """
        Archive les derniers messages dans un fichier texte avec une limite choisie.
        ...
        try:
            with open(log_file, "w", encoding="UTF-8") as f:
                f.write(
                    f'Archived messages from : #{context.channel} ({context.channel.id}) in the server "{context.guild}" ({context.guild.id}) on {datetime.now().strftime("%d.%m.%Y %H:%M:%S")}\n'
                )
                async for message in context.channel.history(
                    limit=limit, before=context.message
                ):
                    f.write(f"[{message.created_at.strftime('%d.%m.%Y %H:%M:%S')}] {message.author}: {message.content}\n")
            
            with open(log_file, "rb") as f:
                file = discord.File(f, filename=f"archive_{context.channel.name}.log")
                await context.send(file=file)
                
        except Exception as e:
            embed = discord.Embed(
                title="Error",
                description=f"An error occurred while archiving: {e}",
                color=0xE02B2B,
            )
            await context.send(embed=embed)
        finally:
            ...
            try:
                ...
                # Si le fichier est encore utilisé, on l'ignore
                pass
    ...
    async def timeout(
        ...
        try:
            ...
            try:
                ...
                # Si le bot ne peut pas envoyer un MP à l'utilisateur, on continue
                pass
